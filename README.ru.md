...
Интерактивный элемент — прямой потомок интерактивного слоя, в одном из двух видов:

- **Плоская фигура** — `<path>`, `<rect>`, `<circle>`… — с `id`. Красится напрямую.
- **Обёртка `<g>`.** Красятся её прямые плоские потомки. Вложенный внутрь `<g>` — рисунок
  дизайнера (иконки, сложная графика), он никогда не перекрашивается, но получает все события
  интерактивного слоя.

Плоские фигуры без `id` — оформление: их нельзя привязать к данным, поэтому они не трогаются.
...
style: {
  ...
  // Снять инлайн-стили из SVG, перебивающие конфиг
...
### Инлайн-стили в SVG (`stripInlineStyles`)

Редакторы выгружают фигуры с атрибутом `style`:

```xml
<rect id="room-101" style="fill:none;stroke:#d5c096"/>
```

Объявление в атрибуте `style` по каскаду стоит выше любого стилшита независимо от специфичности,
поэтому `default`, `hover` и `states` не дают видимого эффекта. Чтобы этот сбой не был молчаливым,
svgic сообщает о нём один раз при инициализации:
...
`stripInlineStyles` удаляет конфликтующие объявления:

```ts
style: {
  default: { fill: '#cfe8ff' },
  hover:   { fill: '#1e88e5' },
  stripInlineStyles: true,   // снимет fill, оставит stroke:#d5c096
}
```

| Значение | Поведение |
|---|---|
| `false` (по умолчанию) | Ничего не удаляется; конфликты выводятся через `console.warn` |
| `true` / `'managed'` | Удаляет только свойства, объявленные в конфиге стилей — объединение `default`, `hover`, `highlightedHover` и всех записей `states` |
| `'all'` | Удаляет атрибут `style` целиком |
| `string[]` | Удаляет ровно перечисленные CSS-свойства |

Затрагиваются только окрашиваемые элементы: сама плоская фигура либо плоские потомки обёртки
`<g>`. Вложенный `<g>` с рисунком не меняется, как и любой другой слой.

`'managed'` — безопасный вариант. Редакторы часто кладут в тот же атрибут `transform`, `display`
или `mix-blend-mode`, и их удаление сломает картинку. `'all'` — для файлов, где в атрибуте нет
ничего, кроме покраски.

Презентационные атрибуты (`fill="#fff"` именно атрибутом, не внутри `style=`) не конфликтуют:
они стоят ниже инжектируемого CSS, и чистить их не нужно.

> Браузер пересобирает остаток атрибута, поэтому `stroke:#d5c096` читается обратно как
> `stroke: rgb(213, 192, 150)`. На отрисовку это не влияет.
...
## ContentPlugin — контент внутри фигур

Размещает текст, изображения и составной контент внутри элементов слоя: позиции считаются
из геометрии каждой фигуры, поэтому правка плана не требует ручной расстановки подписей.
...
так контент деградирует по мере того, как места становится меньше: карточка уступает
названию, название — номеру помещения. Точка выбирается как максимально удалённая от границ
фигуры: центр описывающего прямоугольника в любом Г- или П-образном помещении попал бы на
стену. Подпись, которая помещается только по вертикали, поворачивается на -90°.

Сгенерированный слой не получает события указателя, всё обрезается по своей фигуре, а сам
плагин перерисовывается при каждом `setData()`.

Полное описание: **[docs/api.ru.md — ContentPlugin](docs/api.ru.md#contentplugin)**
...
| `onDataChange(data, client)` | при `setData()` и при инициализации с `options.data` | — |

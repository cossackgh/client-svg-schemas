...
- Роли: `interactive` (события и связывание с данными) | `data` (слой только на чтение, для плагинов) | любая своя строка. Слои, не перечисленные в конфиге, — статичные, ядру не видны

```ts
new Svgic('#container', {
  layers: {
    'rooms':      { role: 'interactive' },
    // background не перечислен — рисуется как статичный SVG
  }
})
```
...
- **Ветки:** любые изменения — только через отдельную ветку. Перед началом работы проверить текущую ветку (`git branch`). Если стоим на `main` — создать ветку и переключиться на неё. Напрямую в `main` не коммитить никогда.
...
- **Язык кода:** все комментарии в коде и JSDoc — на **английском**
- **Коммиты:** описания коммитов — на **английском**. Это правило имеет приоритет над глобальным правилом (русский язык коммитов). Формат: Conventional Commits на английском, например `feat(core): add setSrc() method`, `fix(ui): correct popup positioning`.
- **PR:** заголовок и описание — на **английском**.
- **Документация:** при добавлении нового контента — сначала на английском, затем то же самое на русском (обе версии документации обновляются)

...
| `wheelMode` | `'always'` — зум всегда; `'ctrl'` — только с Ctrl, на macOS также с Cmd (для страниц со скроллом); `'alt'` — только с Alt |
...
- **Собственные трансформации фигур.** Трансформация самой фигуры учитывается: размещение
  прогоняется через неё, поэтому отражённая или повёрнутая на четверть оборота фигура получает
  контент в правильном месте, нужного размера и не перевёрнутым. Поворот на угол, не кратный 90°,
  делает проверку помещаемости приблизительной. Внутри `<g>`-обёртки трансформации самих
  потомков по-прежнему не учитываются.
